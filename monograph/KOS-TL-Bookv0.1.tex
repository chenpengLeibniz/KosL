...
\usetikzlibrary{shapes,arrows,backgrounds,fit,arrows,positioning,calc}
...
\setcounter{tocdepth}{4}   % 目录也显示到第四级（可选）
...
KOS-TL 的最小内核是一个**形式化的决策内核**，它将整个决策过程压缩为五个核心接口。这些接口紧密协作，以确保每个事件、每个状态变化和每个决策都符合严格的规范约束。

\subsection{1. 事件入口接口 (Event Intake Interface)}

事件是系统状态变化的唯一触发器，KOS-TL 对事件有严格的定义，确保每个事件都具备合法性和可验证性。

\subsection{2. 世界状态接口 (World State Interface)}

世界状态表示系统在某一时刻的**合法配置**，状态只能通过合规的事件进行转移，且每个状态都必须满足合法性约束。

\subsection{3. 规则与合规接口 (Normative Rule Interface)}

KOS-TL 的核心思想在于**规范性规则**：每个行动都必须满足合规性要求。规则通过形式化推理提供合法性验证，每个行动都可追溯至具体的规范。

\subsection{4. 执行与轨迹接口 (Trace Interface)}

执行轨迹是 KOS-TL 核心的可重放证据，它记录了事件与规则的执行路径，并能够证明某一状态的构造过程。

\subsection{5. 决策输出接口 (Decision Interface)}
...
设 \\(\\mathcal{K}\\) 为满足上述接口的 KOS-TL 内核。对任意状态 \\(σ\\) 与行动 \\(a\\)，以下命题等价：

\begin{enumerate}
    \item $a$ 在 $\sigma$ 中是合法的（`decide($\sigma$,a) $\neq$ Forbidden`）；
    \item 存在一条有限执行轨迹 $T = \sigma_1 \rightarrow \sigma_2,...,\rightarrow \sigma_n$，其中 $\sigma_n = \sigma$，且a在某一步被实现，并且valid\_trace(T)；
    \item 行动 a 所诱导的后继状态$\sigma'$在 $\mathcal{K}$中是可构造的。
\end{enumerate}

反之，若不存在这样的轨迹，则对应行动或状态在逻辑上不可构造（$\bot$）。
\end{theorem}
